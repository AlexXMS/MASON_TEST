...
public class MyMemory extends TWAgentWorkingMemory{
	...
	public MyMemory(TWAgent moi, Schedule schedule, int x, int y) {
		super(moi, schedule, y, y); // 调用父类构造函数
	...
	public void updateMemory(Bag sensedObjects, IntBag objectXCoords, IntBag objectYCoords, Bag sensedAgents, IntBag agentXCoords, IntBag agentYCoords) {
		...
		for (int x = 0; x < objects.length; x++) {
			for (int y = 0; y < objects[x].length; y++) {
				if (objects[x][y] != null) {
					TWEntity memoryEntity = (TWEntity) objects[x][y].getO();
					double currentTime = this.getSimulationTime();
					double memoryTime = objects[x][y].getT();
					
					// 检查记忆是否过期或实体是否已过期
					boolean isMemoryExpired = currentTime - memoryTime > MAX_TIME;
					boolean isEntityExpired = false;
					
					if (memoryEntity instanceof TWObject) {
						TWObject obj = (TWObject) memoryEntity;
						isEntityExpired = currentTime > obj.getDeathTime();
					}
					
					if (isMemoryExpired || isEntityExpired) {
						objects[x][y] = null;
		...
		for (int i = 0; i < sensedObjects.size(); i++) {
			...
			// 检查实体是否已过期
			TWObject obj = (TWObject) o;
			if (this.getSimulationTime() > obj.getDeathTime()) {
				continue;
			}
			
			// 如果该位置没有记忆或记忆已过期，增加记忆大小
	...
	public void updateFromMessage(MyCommunication message) {
		...
		if (entity != null) {
			if (objects[entity.getX()][entity.getY()] == null) {
				memorySize++;
				System.out.println("Agent " + me.getName() + " 记忆更新: 新增实体 " + 
								 entity.getClass().getSimpleName() + 
								 " 在位置(" + entity.getX() + "," + entity.getY() + ")");
			} else {
				System.out.println("Agent " + me.getName() + " 记忆更新: 更新实体 " + 
								 entity.getClass().getSimpleName() + 
								 " 在位置(" + entity.getX() + "," + entity.getY() + ")");
			}
